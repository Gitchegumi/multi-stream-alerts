...
function makeParsedForm(overrides: Partial<{ verificationToken: string; rawEventId: string }> = {}) {
  return {
...
test("returns 404 when the channel slug doesn't resolve", async () => {
  ...
  assert.deepEqual(warn!.args, ["kofi webhook rejected", { channelSlug: "missing-channel", reason: "channel_not_found" }]);
});
...
test("returns 503 when the channel exists but no Ko-fi token is stored", async () => {
  ...
  const warn = captured.find((c) => c.method === "warn");
  assert.deepEqual(warn!.args, ["kofi webhook rejected", { channelSlug: "alpha", reason: "not_configured" }]);
});
...
test("returns 401 when the inbound token does not match the stored token", async () => {
  ...
  const parseForm = mock.fn((_body: unknown) => makeParsedForm({ verificationToken: "inbound-token-BBBB" }));
  const claimDedup = mock.fn(async () => true);
  const storeAndPublish = mock.fn(async () => makeEvent("channel-1"));
  ...
  const warn = captured.find((c) => c.method === "warn");
  assert.deepEqual(warn!.args, ["kofi webhook rejected", { channelSlug: "alpha", reason: "invalid_token" }]);
});
...
test("returns 200 on a valid first-time event and stores the event", async () => {
  ...
  const publishInput = (publishCall.arguments as unknown as [{ channelId: string; rawEventId: string }])[0];
  ...
  assert.deepEqual(info!.args, ["kofi webhook accepted", { channelSlug: "alpha", rawEventId: "kofi-event-fresh" }]);
});
...
test("returns 200 with { ok: true, duplicate: true } on a duplicate rawEventId", async () => {
  ...
  const getToken = mock.fn(async (_channelId: string) => "stored-secret-token");
  const parseForm = mock.fn((_body: unknown) =>
    makeParsedForm({ verificationToken: "stored-secret-token", rawEventId: "kofi-event-already-seen" })
  );
  ...
  const warn = captured.find((c) => c.method === "warn");
  assert.deepEqual(warn!.args, ["kofi webhook rejected", { channelSlug: "alpha", reason: "duplicate" }]);
});
